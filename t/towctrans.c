            ...
            *status = p[2];
            ...
            if (c) {
                ...
                /* we have 104 unhandled F multi-char mappings */
                    ...
                    if (!ignore_f) {
                        if (lwr != wc)
                            printf("not ok %u towlower(U+%04X) => U+%04X != lower=(%s) F %s\n", i++, wc,
                                   lwr, mapping, name);
                        else
                    ...
                    if (mp != lwr)
                        printf("not ok %u Error towlower(U+%04X) => U+%04X != lower=%s status=%s "
                    ...
                    if (wc != upr) {
                        ...
                        if (wc != upr) {
                            ...
                            for (int i=0; fixups[i][0]; i++) {
                                if (fixups[i][0] == lwr) {
                                    printf("# your libc towupper(0x%x) => 0x%x is wrong\n", lwr, wc);
                                    wc = fixups[i][1];
                                    break;
                                }
                                if (fixups[i][0] > lwr)
                        ...
                        if (wc != upr) {
                            printf("not ok %u Error towupper(U+%04X) => U+%04X != upper=U+%04X %s\n",
                                   i++, lwr, upr, wc, name);
                        } else {
                            printf("ok %u towupper(U+%04X) => U+%04X (cross-checked)\n",
                                   i++, lwr, upr);
                        }
                    } else {
